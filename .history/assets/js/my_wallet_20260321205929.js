    ...
    function renderWallets() {
        ...
        container.innerHTML += `
            <div class="wallet-card-main">
                ...
                <div class="w-details">
                    <span class="text-muted">Tiền mặt (Ví chính)</span>
                    <p id="main-wallet-balance">${formatMoney(mainBalance)}</p>
                </div>
            </div>
        `;
        ...
        customWallets.forEach(w => {
            // Nếu là nợ thì in màu đỏ, ngược lại in màu mặc định
            const colorClass = w.type === 'Nợ' ? 'text-red' : '';
            container.innerHTML += `
                <div class="wallet-card-main">
                    <i class='bx ${icons[w.type] || 'bx-wallet'}'></i>
                    <div class="w-details">
