// ĐỒNG BỘ MÀU TỪ INDEX.HTML SANG TẤT CẢ CÁC TRANG
...
if (savedColor) {
    ...
    document.documentElement.style.setProperty('--user-theme-light', savedColor + '1A'); // Màu nền nhạt
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderWallets() {
        ...
        customWallets.forEach(w => {
            ...
            const colorCode = isDebt ? 'var(--accent-red)' : 'var(--text-main)';
            const borderLeftColor = isDebt ? 'var(--accent-red)' : 'var(--border)';
            
            container.innerHTML += `
                <div class="wallet-card-main" style="border: 1px solid var(--border); border-left: 6px solid ${borderLeftColor};">
                    <div class="icon-box" style="background: ${isDebt ? 'rgba(239, 68, 68, 0.1)' : 'var(--bg-body)'}; color: ${isDebt ? 'var(--accent-red)' : 'var(--text-muted)'};"><i class='bx ${icons[w.type] || 'bx-wallet'}'></i></div>
                    <div class="w-details">
                        <span style="background: var(--bg-body); color: var(--text-muted); padding: 4px 10px; border-radius: 6px; font-size: 0.7rem; font-weight: 700; text-transform: uppercase;">${w.type}</span>
