...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 5. HÀM HIỂN THỊ DANH SÁCH VÍ (GỌI TỪ MYSQL)
    async function renderWallets() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                mainBalance = totalIncome - totalExpense;
            }
        } catch (error) {
        ...
        container.innerHTML += `
            <div class="wallet-card-main" onclick="openDetailModal('${defaultName}', ${mainBalance}, 'Tiền mặt', '${defaultIcon}', true)" style="border: 1px solid var(--primary); border-left: 6px solid var(--primary);">
        ...
        customWallets.forEach(w => {
            ...
            container.innerHTML += `
                <div class="wallet-card-main" onclick="openDetailModal('${w.name}', ${w.balance}, '${w.type}', '${iconClass}', false)" style="border: 1px solid var(--border); border-left: 6px solid ${colorCode};">
...
// --- CÁC HÀM XỬ LÝ CHI TIẾT VÍ ---
async function openDetailModal(name, balance, type, icon, isMain) {
    ...
    switchDetailTab('analytic');
    initDetailChart(balance);
    ...
    if(isMain) {
        ...
        try {
            ...
            if(response.ok) {
                ...
                renderDetailTransactions(listDiv, allTx);
            }
        } catch(e) {
            listDiv.innerHTML = 'Lỗi kết nối';
        }
    } else {
        listDiv.innerHTML = '<div style="text-align:center; padding:30px; color:#94a3b8;">Ví phụ chưa có lịch sử chi tiết.</div>';
    }
}
...
function initDetailChart(balance) {
    ...
    wdChartInstance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Biến động',
                data: [balance*0.05, balance*0.12, balance*0.08, balance*0.2],
...
function renderDetailTransactions(listDiv, allTx) {
    if(allTx.length === 0) {
